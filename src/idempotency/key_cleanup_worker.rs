...
use crate::{error::Z2PResult, configuration::Settings, startup::get_connection_pool};
use sqlx::PgPool;
...
pub async fn run_cleanup_worker_until_stopped(configuration: Settings) -> Z2PResult<()> {
    ...
    worker_loop(connection_pool, configuration.application.idempotency_lifetime_minutes).await
}
